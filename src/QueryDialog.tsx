...
export const QueryDialog: FC<{ connectionString: string }> = ({connectionString}) => {
    console.log("connectionString", connectionString)
    const connectionStringReplace = connectionString.replace('neon.tech/', 'neon.tech:5432/')
    console.log("connectionStringReplace", connectionStringReplace)
    const [query, setQuery] = useState('select table_name from information_schema.tables LIMIT 10;');
    ...
    useEffect(() => {
        const executeQuery = async () => {
            try {
                const res = await axios.post('http://localhost:3002/proxy-postgres', {query, connectionString});
            ...
            <DialogContent className="sm:max">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        ...
                        <Textarea placeholder="Type your SQL query here" onChange={(e) => setQuery(e.target.value)}/>
